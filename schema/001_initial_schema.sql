-- 001 — core tables: profiles, site_settings, brand_settings, properties, leads, appointments
-- Apply: supabase db execute --file schema/001_initial_schema.sql
...
create table if not exists public.brand_settings (
  ...
  logo_light_url   text,
  favicon_url      text,
  primary_color    text not null default '#006aff',
  secondary_color  text not null default '#15181f',
  accent_color     text not null default '#0a53c2',
  hero_image_url   text,
...
create table if not exists public.appointments (
  ...
  appointment_type text not null default 'showing'
                     check (appointment_type in ('showing','consultation','valuation','open_house','call')),
...
create trigger trg_appointments_updated_at
  before update on public.appointments for each row execute function public.set_updated_at();
...
create policy "public read active properties" on public.properties  for select using (active = true);
...
create policy "auth manage brand_settings" on public.brand_settings for all to authenticated using (true) with check (true);
...
create policy "auth update property images"  on storage.objects for update to authenticated using (bucket_id = 'property-images');
...
select 'California Realty Group','Find the home that feels like arrival.','Expert guidance for buyers and sellers across Los Angeles County.'
